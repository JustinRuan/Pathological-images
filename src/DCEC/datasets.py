...
def load_mnist():
    ...
    x = x/255.
    print('MNIST:', x.shape)  #(70000, 28, 28, 1)
    print('MNIST label samples', y.shape)
    return x, y
...
def load_cifar10_edge():
    ...
    x_gray_edge = np.zeros(shape=(60000, 32, 32, 1))
    n = 0
    for i in x:
        # print (i.shape)
        i_gray = rgb2gray(i)
        i_edges = filters.sobel(i_gray)
        i_edges = i_edges.reshape(i_edges.shape[0], i_edges.shape[1], 1).astype('float32')
        x_gray_edge[n] = i_edges
        n += 1
    print(n)
    return x_gray_edge, y
...
def load_usps(data_path='./data/usps'):
    ...
    print('USPS samples', x.shape)  #(9298, 16, 16, 1)
