...
def lagrangian_s3vm_train(xtrain_l,
    ...
    for factor in annealing_sequence:
        ...
        if xtrain_u.shape[0] >= batch_size - l:
            u_batch = xtrain_u[:batch_size - l]
        else:
            u_batch = xtrain_u
        ...
        sample_weight = np.concatenate((l * [1], min(u, batch_size - l) * [factor]))
...
def lagrangian_heuristic(distances,
    ...
    for k in range(iterations):
        ...
        if y_a is not None and y_b is not None:
            lam = planes_intersection(y_a, y_b, u, distances, beta)
        else:
            lam += theta * violation
